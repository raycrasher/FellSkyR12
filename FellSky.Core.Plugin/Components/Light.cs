...
namespace FellSky.Components
{
	...
	public class Light : Component
	{
		...
		public const int MaxVisible = 32;

		private Vector3 dir = Vector3.UnitZ;
		private Vector3 dirTemp = Vector3.UnitZ;
		...
		{
			get { return this.dirTemp; }
			set
			{
				this.dirTemp = value;
				if (this.dirTemp != Vector3.Zero) this.dir = this.dirTemp.Normalized;
				else this.dir = Vector3.Zero;
			}
		}
		...
		public static void UpdateLighting(IDrawDevice device)
		{
			...
			info.PriorizedLights = Duality.Resources.Scene.Current.FindComponents<Light>().Where(l => l.Active).Where(l => l.IsVisibleTo(device)).ToList();
		...
		public static void GetLightAtWorldPos(Vector3 worldPos, out Vector4 lightColor, float translucency = 0.0f)
		{
			...
			IEnumerable<Light> lights = Duality.Resources.Scene.Current.FindComponents<Light>();
