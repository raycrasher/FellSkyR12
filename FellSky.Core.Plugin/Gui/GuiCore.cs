...
{
    public interface ICmpGuiEventHandler
    {
        void HandleGuiEvent(ScriptEventArgs e);
    }

    [EditorHintCategory("Gui")]
    public class GuiCore : Component, ICmpRenderer, ICmpUpdatable, ICmpInitializable, ICmpEditorUpdatable
    {
        ...
        [DontSerialize]
        private List<ScriptEventArgs> eventBuffer = new List<ScriptEventArgs>();
        ...
        public void OnUpdate()
        {
            ...
            {
                foreach (var cmp in Scene.FindComponents<ICmpGuiEventHandler>())
                {
                    foreach (var e in eventBuffer)
                    {
                        cmp.HandleGuiEvent(e);
                    }
                }
                eventBuffer.Clear();
            }
        ...
        public void OnActivate()
        {
            ...
            LibRocketNet.Core.ScriptEvent += Core_ScriptEvent;
        }

        private void Core_ScriptEvent(object sender, ScriptEventArgs e)
        {
            eventBuffer.Add(e);
        }
        ...
        {
            _context?.ProcessMouseMove((int)e.Pos.X, (int)e.Pos.Y, GetKeyModifiers());
        }
        ...
        private void OnKeyDown(object sender, KeyboardKeyEventArgs e)
        {
            if (e.Key == Key.F8)
            {
                ...
                Logs.Game.Write($"LibRocket debug mode {LibRocketNet.Core.DebugMode}");
            }
        ...
        public static void InitializeGui()
        {
            ...
            LibRocketNet.Core.SystemInterface = _systemInterface;
            LibRocketNet.Core.RenderInterface = _renderInterface;
            LibRocketNet.Core.Initialize();
            _context = LibRocketNet.Core.CreateContext("DualityMainContext", new Vector2i(1000, 1000), _renderInterface);
            LibRocketNet.Core.InitDebugger(_context);
            Logs.Game.Write("LibRocket initialized.");

            var fontfaces = JsonConvert.DeserializeObject<string[]>(File.ReadAllText("data/fonts/fontfaces.json"));

            foreach (var font in fontfaces)
            {
                LibRocketNet.Core.LoadFontFace(font);
            }
        }
        ...
        CullingInfo _cullingInfo = new CullingInfo
        {
            Position = Vector3.Zero,
            Radius = 1000000f,
            Visibility = VisibilityGroup
        };
        private static VisibilityFlag visibilityGroup;
    }
}
